...
AveRFU3 = function(popFreq,evidData,refData,hypData,
  ...
  for(hyprow in 1:nrow(hypData)) { 
    ...
    nAu = 0 #number of alleles not belonging to any referencess
    hAu = 0 #Height of alleles not belonging to any refs
    
    nA_refs = rep(0,nRefs)  #number of (unique) alleles for references
    for(loc in locs)  {
      subdat = dat$samples[[1]][[loc]]  #obtain marker info
      evid0 = subdat$adata
      refsLoc = lapply(refUseList,function(x) x[[loc]]$adata) #obtain data for references
      
      ref0 = unlist( refsLoc ) #obtain alleles of referneces
      ind0 = !evid0%in%ref0 #missing inds in evid
      nAu <- nAu + sum(ind0)
      hAu <- hAu + sum(subdat$hdata[ind0] )
      
      #OBTAIN NUMBER OF UNIQUE::
      nAG = sapply(refsLoc,length)
      reftab = cbind(rep(names(refsLoc),nAG),unlist(refsLoc)) #show alleles in table
      reftab = reftab[reftab[,2]%in%evid0,,drop=FALSE] #only those present in evid0
      reftabCount = table(factor(reftab[,1],refUse), reftab[,2]) #obtain count table
      isUnique = colSums(reftabCount>=1)==1 #check if only one contributor has the allele (non-shared)
      
      insAdd = apply(reftabCount[,isUnique,drop=FALSE],2,function(x) which(x==1)) #obtain index to add 1 for
      for(k in 1:length(insAdd)) nA_refs[insAdd[k]] = nA_refs[insAdd[k]] + 1 #add
    }
